...
currentAccount: any;
    ...
    loadAll() {
        this.situationMatrimonialeService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<SituationMatrimoniale[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/situation-matrimoniale'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: SituationMatrimoniale) {
        return item.id;
    }
    registerChangeInSituationMatrimoniales() {
        this.eventSubscriber = this.eventManager.subscribe('situationMatrimonialeListModification', (response) => this.loadAll());
    }
