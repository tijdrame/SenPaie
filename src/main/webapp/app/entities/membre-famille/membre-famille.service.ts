...
export class MembreFamilleService {
    ...
    findByCollab(id: Collaborateur): Observable<HttpResponse<MembreFamille[]>> {
        return this.http.get<MembreFamille[]>(`${this.resourceUrl+"-collab"}/${id}`, { observe: 'response'})
            .map((res: HttpResponse<MembreFamille[]>) => this.convertArrayResponse(res));
    }

    search(prenom?: string, nom?: string, tel?:string, deleted?:boolean): Observable<HttpResponse<MembreFamille[]>>{
        return this.http.get<MembreFamille[]>(`${this.resourceUrl+"Ter"}/${prenom}/${nom}/${tel}/${deleted}`, { observe: 'response'})
            .map((res: HttpResponse<MembreFamille[]>) => this.convertArrayResponse(res));
    }
