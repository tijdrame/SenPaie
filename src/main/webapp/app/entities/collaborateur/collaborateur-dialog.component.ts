...
export class CollaborateurDialogComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.fonctionService.query()
            .subscribe((res: HttpResponse<Fonction[]>) => { this.fonctions = res.body; }, (res: HttpErrorResponse) => this.onError(res.message));
    ...
    trackRegimeById(index: number, item: Regime) {
        return item.id;
    }

    trackSexeById(index: number, item: Sexe) {
        return item.id;
    }
}
