...
public class PiecesResource {
    ...
    public ResponseEntity<Pieces> updatePieces(@Valid @RequestBody Pieces pieces) throws URISyntaxException {
        ...
        Pieces result = piecesService.update(pieces);
    ...
    @GetMapping("/piecesTer/{prenom}/{nom}/{matricule}/{deleted}")
    @Timed
    @Secured({AuthoritiesConstants.ADMIN, AuthoritiesConstants.RH, AuthoritiesConstants.DG})
    public ResponseEntity<List<Pieces>> search(@PathVariable Optional<String> prenom  , @PathVariable Optional<String> nom,
                                               @PathVariable Optional<String> matricule, @PathVariable Boolean deleted, Pageable pageable) {
        log.debug("REST request to get a page of Collab====>: "+prenom.isPresent()+" val"+prenom.get());
        Page<Pieces> page = piecesService.findByCriteres(prenom.isPresent()?"%"+prenom.get().trim()+"%":"",
            nom.isPresent()?"%"+nom.get().trim()+"%":"", matricule.isPresent()?"%"+matricule.get().trim()+"%":"", deleted, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/pieces");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
}
