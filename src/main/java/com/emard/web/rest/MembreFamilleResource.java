...
public class MembreFamilleResource {
    ...
    public ResponseEntity<MembreFamille> updateMembreFamille(@Valid @RequestBody MembreFamille membreFamille) throws URISyntaxException {
        ...
        MembreFamille result = membreFamilleService.update(membreFamille);
    ...
    @GetMapping("/membre-famillesTer/{prenom}/{nom}/{tel}/{deleted}")
    @Timed
    @Secured({AuthoritiesConstants.ADMIN, AuthoritiesConstants.RH, AuthoritiesConstants.DG})
    public ResponseEntity<List<MembreFamille>> search(@PathVariable Optional<String> prenom  , @PathVariable Optional<String> nom,
        ...
        Page<MembreFamille> page = membreFamilleService.findByCriteres(prenom.isPresent()?"%"+prenom.get().trim()+"%":"",
