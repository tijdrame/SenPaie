...
public class StructureResource {
     ...
     * GET  /structures/:id : get the "id" structure.
     *
     * @param code the id of the structure to retrieve
     * @return the ResponseEntity with status 200 (OK) and with body the structure, or with status 404 (Not Found)
     */
    @GetMapping("/structuresDenom/{code}")
    @Timed
    @Secured({AuthoritiesConstants.ADMIN, AuthoritiesConstants.RH, AuthoritiesConstants.DG, AuthoritiesConstants.USER})
    public ResponseEntity<Structure> getStructureByDenomination(@PathVariable String code) {
        log.debug("REST request to get Structure : {}", code);
        Structure structure = structureService.findByDenomination(code);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(structure));
    }
