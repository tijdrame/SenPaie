...
public class DetailPretResource {
    ...
    public DetailPretResource(DetailPretService detailPretService, RemboursementService remboursementService) {
    ...
    public ResponseEntity<DetailPret> createDetailPret(@Valid @RequestBody DetailPret detailPret) throws URISyntaxException {
        ...
        log.debug("Obj detail ==="+detailPret);
        log.debug("Obj resul ==="+result);
        Integer nbRemb = result.getPret().getNbRemboursement();
        Double montant = result.getMontant()/nbRemb;
        LocalDate ld = result.getPret().getDateDebutRemboursement();
        for(int i=0; i<nbRemb;i++){
            Remboursement remboursement = new Remboursement();
            remboursement.deleted(false).detailPret(detailPret).montant(montant).
